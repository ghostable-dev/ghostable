...
class GetSuggestedRuleKeys
{
    ...
    public function handle(Environment $environment): array
    {
        ...
        $grouped = collect($registry->all())
            ->reject(fn ($def) => $existingKeys->contains(strtoupper($def->key())))
            ->groupBy(fn ($def) => $def->group()->name)
            ->map(fn ($defs) => $defs->map->key()->values()->all());

        // 2. Get keys from other environments in the same project
        $projectKeys = $environment->project->environments()
            ...
            ->reject(fn ($key) => $registry->get($key)) // already known in registry
            ->values();
