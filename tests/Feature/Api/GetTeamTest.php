...
test('returns team for member user', function () {
    ...
    $response = $this->getJson('/api/v1/teams/'.$ray->personalTeam()->id);
    $response->assertOk()->assertJsonFragment(['id' => $ray->personalTeam()->id]);
});
...
test('users cannot view teams they do not belong to', function () {
    ...
    Sanctum::actingAs($ray);
    $response = $this->getJson('/api/v1/teams/'.$peter->personalTeam()->id);
...
test('returns team in the correct format', function () {
    $ray = $this->createUser(name: 'Ray', email: 'ray@ghostbusters.com');
    Sanctum::actingAs($ray);
    $this->getJson('/api/v1/teams/'.$ray->personalTeam()->id)
        ->assertOk()
        ->assertJsonStructure([
            'data' => [
                'id',
