...
$classes = Flux::classes()
    ->add('h-10 lg:h-8 relative flex items-center gap-3 rounded-lg overflow-visible')
    ->add('ps-3 before:content-[\'\'] before:absolute before:inset-y-1 before:w-[4px] before:rounded-full before:bg-brand before:opacity-0 before:transition')
    ->add('before:left-[-8px]')
    // only show when current (no hover preview)
    ->add('data-current:before:opacity-100')
    ->add($square ? 'px-2.5!' : '')
