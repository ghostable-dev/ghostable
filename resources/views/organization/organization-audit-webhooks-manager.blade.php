<div class="space-y-6 max-w-4xl" data-screenshot-frame="organization-audit-webhooks">
    <x-section>
        ...
        @if(! $this->canAccessAuditWebhooks)
            <flux:callout color="slate" icon="credit-card">
                <flux:callout.heading>{{ __('Scale plan required') }}</flux:callout.heading>
                <flux:callout.text>
                    {{ __('Signed audit webhooks and SIEM forwarding are available on Scale and Enterprise plans.') }}
                </flux:callout.text>
            </flux:callout>
        @elseif(! $this->canManageAuditWebhooks)
