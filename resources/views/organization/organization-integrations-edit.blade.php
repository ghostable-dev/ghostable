    ...
    <x-layouts.organization-settings>
        <div class="space-y-6 max-w-2xl">
            <div class="flex items-center justify-between gap-4">
                <div>
                    <flux:heading size="lg" level="2">{{ __('Edit Integration') }}</flux:heading>
                    <flux:subheading>{{ __('Update OAuth configuration for your integration.') }}</flux:subheading>
                </div>
                <flux:button
                    variant="ghost"
            ...
            @else
                <x-section>
                    ...
                    <x-slot:subheading>{{ __('Update your integration configuration.') }}</x-slot:subheading>
                    <x-slot:actions>
                        <div class="flex items-center justify-end gap-4">
                            <flux:button variant="primary" wire:click="updateIntegrationClient">
                                {{ __('Save changes') }}
                            </flux:button>
                        </div>
                    </x-slot:actions>
                    <form class="w-full space-y-6">
                        ...
                        <div class="space-y-2">
                            <flux:input
                                ...
                                :required="empty($integrationClient->logo_path)"
                                accept="image/*"/>
                            @if($integrationClient->logoUrl())
                                <img src="{{ $integrationClient->logoUrl() }}" alt="{{ $integrationClient->name }} logo" class="h-16 w-16 rounded-lg bg-white object-cover ring-1 ring-slate-200">
                            @endif
                            @error('logo')
                ...
                <x-section>
                    ...
                    <x-slot:subheading>{{ __('Client ID is fixed. Client secret is only shown when created.') }}</x-slot:subheading>
                    <div class="w-full space-y-4">
                        <flux:input label="Client ID" readonly copyable value="{{ $integrationClient->client_id }}"/>
                    </div>
                </x-section>
            @endif
        </div>
