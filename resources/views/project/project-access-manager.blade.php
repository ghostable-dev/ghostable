<x-layouts.project-settings :project="$this->project">
    ...
    @if($this->project->owningOrganization()->activeSubscription())
        @can('manageAccessControls', $this->project->owningOrganization())
            {{-- Toggle project restricted access --}}
            <div>
                <div class="mb-4">
                    <flux:heading size="lg">{{ __('Access Restrictions') }}</flux:heading>
                    <flux:subheading>
                        {{ __('When enabled, organization members will not be able to access this 
                        project unless they are explicitly granted access in Access Control.') }}
                    </flux:subheading>
                </div>
